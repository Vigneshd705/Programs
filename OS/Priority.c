#include <stdio.h>
int n, burst_time[100], wait_time[100], turn_around_time[100], process[100], priority[100];
int waittime()
{
    wait_time[0] = 0;
    for (int j = 1; j < n; j++)
...
void main()
{
    ...
    printf("\n");
    turnaroundtime();
    printf("Process No.    Burst Time  Waiting Time  Turn around time       Priority\n");
    for (int i = 0; i < n; i++)
    {
        printf("    %-15d %-13d  %-12d  %-15d %d\n", process[i], burst_time[i], wait_time[i], turn_around_time[i], priority[i]);
    }
